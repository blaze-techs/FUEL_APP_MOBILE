...
export function etimsConfig(){
  ...
  if(!baseUrl||!tin||!deviceSerial) throw new Error("eTIMS server configuration is incomplete");
  return {baseUrl,invoicePath,tin,bhfId,deviceSerial,authToken,mode};
}

export async function submitEtimsDocument(payload:unknown):Promise<{ok:boolean,status:number,body:any}>{
  const c=etimsConfig();
  ...
  let body:any=null;
  try{body=await res.json();}catch{body={raw:await res.text().catch(()=>null)};}
...
export function buildEtimsInvoice(input:{
  ...
  const c=etimsConfig();
  return {
    tin:c.tin,bhfId:c.bhfId,deviceSerial:c.deviceSerial,mode:c.mode,
    invoiceNumber:input.invoiceNumber,saleDate:input.saleDate,
    customer:{pin:input.customerPin||null,name:input.customerName||null},
    totals:{gross:input.total,tax:input.taxAmount,net:input.total-input.taxAmount,currency:"KES"},
    items:input.items.map((i,idx)=>({
      lineNumber:idx+1,name:i.name,quantity:i.quantity,unitPrice:i.unitPrice,
      lineTotal:Number((i.quantity*i.unitPrice).toFixed(2)),taxCode:i.taxCode||"B"
    }))
  };
}
