...
import { mpesaBaseUrl, mpesaConfig, mpesaPassword, mpesaTimestamp, mpesaToken } from "../_lib/mpesa.js";
...
export async function POST(request:Request):Promise<Response>{
  try{
    ...
    const { data:tx,error }=await supabaseAdmin.from("payment_transactions").select("*").eq("checkout_request_id",checkout).maybeSingle();
    ...
    try{
      ...
      const res=await fetch(`${mpesaBaseUrl()}/mpesa/stkpushquery/v1/query`,{
        ...
        body:JSON.stringify({BusinessShortCode:c.shortcode,Password:mpesaPassword(timestamp),Timestamp:timestamp,CheckoutRequestID:checkout})
      });
      remote=await res.json();
    }catch{/* callback remains authoritative */}
