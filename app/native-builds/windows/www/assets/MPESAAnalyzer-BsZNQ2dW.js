...
import { C as Q } from "./calculator-CExr6nLN.js";
import { B as Qe } from "./ban-_x9vE7Pf.js";
/**
  ...
  const [E, Re] = m.useState("pdf"),
    ...
    De = t => {
      ...
      for (let d = 0; d < t.length; d++) {
        ...
        const U = V.join(" ");
        let I = "";
        const fe = n.includes("Online") || U.includes("Online"),
          ye = U.match(/((?:254)?\d{2,4}\*+\d{3})/),
          je = ye ? ye[1] : "",
          Ne = U.match(
            /(?:\d{3,4}\*+\d{3}|254\d{0,3}\*+\d{3})\s*-\s*(.+?)(?:\s+Merchant\s+PUBLICAN|\s+Payment\s+ENERGY|$)/i
          );
        if (Ne) {
          let p = Ne[1].trim();
          p = p
            .replace(
              /\s+(ENERGY|PUBLICAN|SWAFIA|Customer|Merchant|Payment)\s*$/gi,
              ""
            )
            .trim();
        ...
        const ke =
            n.match(/(\d{2}:\d{2}:\d{2})/) || U.match(/(\d{2}:\d{2}:\d{2})/),
          Ke = ke ? ke[1] : "";
        a.push({
          details: I,
          paidIn: h,
...
The PDF text may not have been extracted correctly. Try the "Manual Text Paste" method: open the PDF in a viewer, select all text, copy, and paste it here.`);
    ...
    be = async t => {
      ...
      const a = await Le(t),
        s = ge(a, []);
      (F(a), $(s), b(`AI complete! ${a.length} inflows found`));
    },
    _e = t => {
    ...
    children: [
      ...
      e.jsx("div", {
        ...
        children: [
          ...
          { id: "ai", label: "AI Only", desc: "Gemini AI extraction", icon: _ },
        ].map(({ id: t, label: a, desc: s, icon: d }) =>
          e.jsxs(
            "button",
            {
              onClick: () => Re(t),
              className: `flex-1 px-4 py-3 rounded-xl text-xs font-semibold transition-all ${E === t ? "bg-green-600 text-white shadow-lg" : "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50"}`,
              children: [
        ...
        e.jsxs("div", {
          ...
          children: [
            e.jsx("input", {
              ...
              multiple: !0,
              onChange: t => {
                (_e(t.target.files), (t.target.value = ""));
              },
              className: "hidden",
            }),
...
18:26:32 0746***921 - isaac alemu aleper Merchant PUBLICAN ENERGY
Payment ENERGY
...`,
              className:
          ...
          children: [
            ...
            e.jsx("button", {
              ...
              children: w
                ...
                : e.jsxs(e.Fragment, {
                    children: [e.jsx(_, { size: 18 }), " Extract with AI"],
                  }),
            }),
          ],
        }),
