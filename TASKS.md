...
## ✅ TASK-2026-09-15-009: In-game CONTROLS system — keyboard / mouse / controller with auto-detect + gamepad bridge

**Problem** — "controls aren't working" across ALL browser game embeds. Root causes: (1) the game iframe was never focused, so keystrokes went to the parent page until the user physically clicked inside the game; (2) there was no way to pick which input to use, no auto-detection, and no controller support for the same-origin mirror games.

| Action      | File / commit                                     | Detail                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Fix         | `src/react-app/lib/game-controls.ts` (new, +~340) | Focus: `focusGameFrame` autofocuses the game frame on open/load + on any click on the playing surface; `forwardKeysToFrame` is a capture-phase parent key-forwarder so even the FIRST keypress lands in the game. Control modes: `ControlMode` = auto/keyboard/mouse/gamepad; `detectControls` polls `navigator.getGamepads` + pointer media queries. Gamepad→keyboard bridge: `startGamepadBridge` polls pads and dispatches synthetic `KeyboardEvent`s into same-origin `contentDocument` (`GAMEPAD_BUTTON_KEYS`/`GAMEPAD_AXIS_KEYS` map A/B/X/Y/d-pad/LB/RB/LT/RT/start + sticks → space/e/Shift/q/w/r/f/Enter/arrows + wasd). `isSameOriginFrame` treats relative mirror paths (`/api/game-embed/*`, `/api/quenq-embed/*`) as same-origin by definition. |
| Fix         | `src/react-app/components/VideoGames.tsx`         | `UnifiedPlayer` wired: `frameRef`, control-mode + status + controls-popover + bridge-active state, auto-focus effect with retry, key-forwarder effect, gamepad-bridge effect (Auto/Controller modes, same-origin only), Controls picker in the header (4 mode chips + live gamepad/touch/keyboard status + bridge ON badge), one-time "click to focus" hint (auto-dismisses 4.5s), bottom bar shows live status or "Controller". iframe keeps `allow="fullscreen; autoplay; gamepad; …"` + `allowFullScreen`.                                                                                                                                                                                                                                                |
| Tests       | `src/test/game-controls.test.ts` (new, 19 tests)  | detect (gamepad count/touch/privacy-mode), statusLabel, isSameOriginFrame (relative vs absolute vs Node), focusGameFrame, forwardKeysToFrame (focus + re-dispatch + inert when focused), pressedKeysFromGamepad (buttons/axes/deadzone), dispatchKeyIntoFrame (doc+window, cross-origin false), startGamepadBridge (same-origin keydown/keyup edges; cross-origin inert).                                                                                                                                                                                                                                                                                                                                                                                    |
| E2E         | `e2e/controls.spec.ts` (new)                      | Node-side: shipped logic exposes 4 modes, classifies origins, maps gamepad→keys, detectControls safe. Live-host: deployed bundle carries Controls UI markers ("Game controls", "Controller", "Mouse / touch", "gamepadCount") on BOTH hosts; quenq + crazy mirrors 200; injected mirror iframe boots with zero ad requests + `gamepad` permission-policy token.                                                                                                                                                                                                                                                                                                                                                                                              |
| Committed   | GitHub `82a5ab8` + `99a9368`                      | `feat(games): in-game Controls system…` and `test(games)+fix(controls): E2E controls spec + robust relative-path origin detection`. Pushed `30b3167..99a9368`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| CI / Deploy | GitHub Actions, Vercel, Cloudflare                | CI "Continuous Integration" for `99a9368` = **success**; "Deploy" (CF Pages) = **success** (pages.dev live). Vercel rebuild `vercel build --prod` exit 0 → `vercel deploy --prebuilt --prod` → alias `fuel-app-mobile.vercel.app` → `ns3jtdo9a` (chunk `VideoGames-CI-3LOIp.js`). Supabase: no schema/API changes needed.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| E2E result  | `e2e/controls.spec.ts` (chromium, both hosts)     | Node-side group **4/4 passed**. Live-host group run after CF+Vercel deploy: bundle markers + mirror boot + gamepad token + `adRequests=0` verified (see session log). Earlier run failed only on missing Playwright chromium binary (`npx playwright install chromium` re-installed; ~115 MB).                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Quality     | –                                                 | vitest **466 passed / 6 skipped** (19 new), `tsc -b` 0, eslint 0 errors, prettier clean, `build:static` exit 0.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

**How it works** — An iframe only receives keyboard/mouse input while IT is focused; the player now auto-focuses it (open/load/click) and forwards stray keys, so controls always reach the game. A Controls button in the player header lets the user switch Auto / Keyboard / Mouse / Controller; auto-detection reports live gamepad count + touch vs mouse. For same-origin mirrors (our ad-free `/api/game-embed` + `/api/quenq-embed` games) a gamepad→keyboard bridge maps a physical controller to synthetic key events in the game document, so ANY gamepad plays keyboard-only games. Cross-origin embeds (archive.org, classic.minecraft.net) keep their own native gamepad support (the iframe still carries the `gamepad` permission-policy token) with an honest "can't bridge" note. NO ADS touched — verified by E2E (`adRequests=0`).
