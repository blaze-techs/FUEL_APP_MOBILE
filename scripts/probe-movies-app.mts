...
const bad = [], errs = [];
pg.on('response', (r) => { if (r.status() >= 400) bad.push(r.status() + ' ' + r.url().slice(0, 110)); });
...
await pg.getByText(/Skip tour|Skip/i).first().click({ timeout: 4000 }).catch(() => {});
...
await moviesBtn.click({ timeout: 15000 }).catch(() => console.log('movies click failed'));
...
console.log('MOVIES_SECTION:', body.slice(body.search(/Movies/i), body.search(/Movies/i) + 400));
