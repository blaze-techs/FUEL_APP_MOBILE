...
export function mpesaBaseUrl():string{
  return (process.env.MPESA_ENVIRONMENT||"production").toLowerCase()==="sandbox"
...
export function mpesaConfig(){
  ...
  if(!consumerKey||!consumerSecret||!passkey||!shortcode||!callbackUrl) throw new Error("M-Pesa server credentials are incomplete");
...
export async function mpesaToken():Promise<string>{
  ...
  const res=await fetch(`${mpesaBaseUrl()}/oauth/v1/generate?grant_type=client_credentials`,{headers:{Authorization:`Basic ${auth}`}});
...
export async function auditServer(stationId:string|null,action:string,entityType:string,entityId:string|null,newValues:any){
  if(!supabaseAdmin) return;
  await supabaseAdmin.from("immutable_audit_log").insert({
    station_id:stationId,user_id:null,action,entity_type:entityType,entity_id:entityId,new_values:newValues
  });
}
