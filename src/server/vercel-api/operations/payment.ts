...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (
      !["cash", "card", "bank", "credit", "voucher", "other"].includes(method)
    )
    ...
    const providerReference = String(
      body.providerReference || `${provider}-${idempotencyKey}`,
    );
    ...
    const { data: existing } = await supabaseAdmin
      ...
      .maybeSingle();
    if (existing)
      return json({ success: true, duplicate: true, data: existing });

    if (body.saleId) {
      const { data: sale, error: saleError } = await supabaseAdmin
        .from("sales_ledger")
        .select("id,station_id,shift_id,gross_amount")
        .eq("id", body.saleId)
        .maybeSingle();
      if (saleError || !sale)
        throw Object.assign(
          new Error(saleError?.message || "Canonical sale not found"),
          { status: 404 },
        );
      if (sale.station_id !== stationId)
        throw Object.assign(new Error("Sale belongs to another station"), {
          status: 409,
        });
    ...
    if (method === "cash" && body.drawerId) {
      const { error: cashError } = await supabaseAdmin
        ...
        .insert({
          drawer_id: body.drawerId,
          station_id: stationId,
          event_type: "sale",
          amount,
          reference_type: "payment_transaction",
          reference_id: data.id,
          note: body.note || "Canonical cash sale",
