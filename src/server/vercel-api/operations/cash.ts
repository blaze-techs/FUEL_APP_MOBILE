  ...
  try {
    ...
    if (action === "open") {
      ...
      if (!Number.isFinite(opening) || opening < 0)
        throw Object.assign(new Error("Invalid opening cash"), { status: 400 });
    ...
    if (action === "event") {
      ...
      if (!Number.isFinite(amount) || amount === 0)
        throw Object.assign(new Error("amount must be non-zero"), {
          status: 400,
        });
      const { data, error } = await supabaseAdmin
    ...
    if (action === "close") {
      ...
      const { data: drawer, error: lookupError } = await supabaseAdmin
        .from("cash_drawers")
        .select("*")
        .eq("id", body.drawerId)
      ...
      if (!drawer || drawer.status !== "open")
        throw Object.assign(new Error("Open drawer not found"), {
          status: 404,
        });
      ...
      if (!Number.isFinite(counted) || counted < 0)
        throw Object.assign(new Error("Invalid counted cash"), { status: 400 });
      ...
      if (Math.abs(variance) > 100) {
        await supabaseAdmin.from("anomaly_events").insert({
          ...
          severity: Math.abs(variance) > 5000 ? "critical" : "high",
