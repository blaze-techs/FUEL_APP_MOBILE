...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const ctx = await authenticate(request),
      b: any = await request.json(),
      a = new URL(request.url).searchParams.get("action") || "",
    ...
    if (a === "open") {
      ...
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          opening_cash: Number(b.openingCash || 0),
        })
        .select("*")
        .single();
    ...
    if (a === "event") {
      ...
      if (!Number.isFinite(amount) || amount === 0)
        throw Object.assign(new Error("Invalid drawer amount"), {
          status: 400,
        });
      const { data, error } = await supabaseAdmin
        .from("cash_drawer_events")
        .insert({
          drawer_id: b.drawerId,
          station_id: stationId,
          event_type: b.eventType,
          amount,
          reference_type: b.referenceType || null,
    ...
    if (a === "close") {
      ...
      const { data, error } = await supabaseAdmin.rpc(
        "fuelpro_close_cash_drawer",
        {
          p_drawer: b.drawerId,
          p_counted: Number(b.countedCash),
