  ...
  try {
    ...
    if (action === "post") {
      ...
      const { data, error } = await supabaseAdmin.rpc("fuelpro_post_sale", {
        ...
        p_shift: body.shiftId || null,
        p_nozzle: body.nozzleId || null,
        p_litres: Number(body.litres),
        p_unit_price: Number(body.unitPrice),
        p_tax: Number(body.taxAmount || 0),
        p_payment_status: body.paymentStatus || "unpaid",
        p_customer: body.customerId || null,
        p_vehicle: body.vehicleRef || null,
        p_receipt: body.receiptNumber || null,
        p_external_ref: body.externalReference || null,
      ...
      if (
        (body.paymentStatus || "unpaid") === "paid" &&
        body.createPayment !== false
      ) {
        const paymentMethod = String(
          body.paymentMethod || "cash",
        ).toLowerCase();
        const idempotencyKey = String(
          body.paymentIdempotencyKey ||
            `${body.idempotencyKey || data.id}:payment`,
        );
        const { error: paymentError } = await supabaseAdmin
          ...
          .insert({
            ...
            provider_reference:
              body.paymentReference || `${paymentMethod}-${idempotencyKey}`,
        ...
        if (paymentError && paymentError.code !== "23505") {
          throw new Error(
            `Sale posted but payment recording failed: ${paymentError.message}`,
          );
        }
      }
    ...
    if (action === "reverse") {
      ...
      const { data, error } = await supabaseAdmin.rpc("fuelpro_reverse_sale", {
        p_sale: body.saleId,
        p_reason: body.reason || "Reversal",
        p_idempotency_key: body.idempotencyKey || crypto.randomUUID(),
      });
      if (error) throw Object.assign(new Error(error.message), { status: 400 });
      ...
      for (const payment of linkedPayments || []) {
        const { error: reversePaymentError } = await supabaseAdmin
          ...
          .update({
            ...
            metadata: {
              reversal_sale_id: data.id,
              reversal_reason: body.reason || "Sale reversal",
            },
          })
          .eq("id", payment.id);
