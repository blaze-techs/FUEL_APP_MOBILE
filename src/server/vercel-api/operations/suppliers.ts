  ...
  try {
    ...
    if (action === "create-po") {
      ...
      const total = items.reduce(
        (s: number, i: any) =>
          s + Number(i.quantity || 0) * Number(i.unitCost || 0),
        0,
      );
      const { data: po, error } = await supabaseAdmin
        ...
        .insert({
          ...
          order_number: body.orderNumber || `PO-${Date.now()}`,
          status: body.status || "draft",
          currency: body.currency || "KES",
          ordered_amount: total,
        ...
        const { error: itemError } = await supabaseAdmin
          ...
          .insert(
            items.map((i: any) => ({
              ...
              fuel_type_id: i.fuelTypeId || null,
    ...
    if (action === "receive") {
      ...
      const { data: delivery, error } = await supabaseAdmin
        ...
        .insert({
          ...
          tanker_registration: body.tankerRegistration || null,
      ...
      for (const i of body.items || []) {
        let tankMovementId: null | string = null,
          inventoryMovementId: null | string = null;
        if (i.itemId) {
          ...
          if (poItem) {
            const nextReceived =
              Number(poItem.received_quantity || 0) + Number(i.quantity || 0);
        ...
        if (i.fuelTypeId) {
          const { data: m, error: me } = await supabaseAdmin
            ...
            .insert({
              station_id: stationId,
              fuel_type_id: i.fuelTypeId,
              tank_id: i.tankId || null,
              movement_type: "delivery",
              quantity_litres: Number(i.quantity),
        ...
        await supabaseAdmin.from("supplier_delivery_items").insert({
          ...
          quantity: Number(i.quantity),
          unit_cost: i.unitCost ?? null,
          tank_movement_id: tankMovementId,
      ...
      if (body.purchaseOrderId) {
        ...
        const fullyReceived = (remaining || []).every(
          (row: any) =>
            Number(row.received_quantity || 0) >= Number(row.quantity || 0),
        );
        await supabaseAdmin
          .from("purchase_order_ledger")
    ...
    return json({
      ...
      data: (orders || []).map((o: any) => ({
        ...
        suppliers: o.supplier_id
          ? supplierMap.get(o.supplier_id) || null
          : null,
      })),
    });
  } catch (e) {
