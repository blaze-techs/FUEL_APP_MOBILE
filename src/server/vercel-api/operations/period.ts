  ...
  try {
    ...
    if (action === "lock") {
      ...
      if (
        ...
        throw Object.assign(
          new Error("periodStart and periodEnd are required"),
          { status: 400 },
        );
      ...
      if ((openShifts.count || 0) > 0)
        throw Object.assign(
          new Error("Cannot lock period with open or unapproved shifts"),
          { status: 409 },
        );
      if ((pendingPayments.count || 0) > 0)
        throw Object.assign(
          new Error("Cannot lock period with pending payments"),
          { status: 409 },
        );
      if ((syncProblems.count || 0) > 0)
        throw Object.assign(
          new Error("Cannot lock period with unresolved sync items"),
          { status: 409 },
        );
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          period_start: start,
          period_end: end,
          reason: body.reason || "Month-end close",
    ...
    if (action === "unlock") {
      ...
      const { data: lock } = await supabaseAdmin
        ...
        .maybeSingle();
      if (!lock)
        throw Object.assign(new Error("Period lock not found"), {
          status: 404,
        });
      await supabaseAdmin.from("immutable_audit_log").insert({
        station_id: stationId,
        ...
        entity_id: lockId,
        old_values: lock,
        new_values: { reason: body.reason || "Authorized reopen" },
      });
