...
const roles = ["owner", "manager", "supervisor", "cashier", "attendant", "accountant", "auditor"] as const;
...
async function authorizeRoleMutation(ctx: Awaited<ReturnType<typeof authenticate>>, stationId: string, targetRole: string) {
  ...
  if (targetRole === "owner") {
    throw Object.assign(new Error("Owner cannot be assigned through the role endpoint"), { status: 403 });
  }
  if (actorRole !== "founder" && actorRole !== "admin") {
    ...
    if (!actorRank || targetRank >= actorRank) {
      throw Object.assign(new Error("You may only assign roles below your own role"), { status: 403 });
    }
  }
  return actorRole;
}
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (userError || !targetUser.user) {
      throw Object.assign(new Error("Target user does not exist"), { status: 404 });
    }
...
export async function DELETE(request: Request): Promise<Response> {
  try {
    ...
    const stationId = clean(body.stationId);
    const userId = clean(body.userId);
    if (!stationId || !userId) throw Object.assign(new Error("stationId and userId are required"), { status: 400 });
    ...
    if (actorRole !== "founder" && actorRole !== "admin") {
      ...
      if (!targetRole || targetRole === "owner" || targetRank >= actorRank) {
        throw Object.assign(new Error("You may only revoke roles below your own role"), { status: 403 });
      }
    }
