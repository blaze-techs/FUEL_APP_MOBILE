...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const accountReference = String(body.accountReference || "FuelPro").slice(
      0,
      12,
    );
    const description = String(body.description || "Fuel purchase").slice(
      0,
      13,
    );
    ...
    const { data: tx, error } = await supabaseAdmin
      ...
      .insert({
        ...
        merchant_request_id: daraja.MerchantRequestID,
    ...
    return json({
      ...
      responseDescription: daraja.ResponseDescription,
