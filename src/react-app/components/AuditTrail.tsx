...
export default function AuditTrail({ stationId }: { stationId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          "id,station_id,user_id,action,entity_type,entity_id,old_values,new_values,created_at",
        )
      ...
      toastError(
        "Could not load the immutable audit log: " + (e as Error).message,
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          setEntries((prev) =>
            [payload.new as AuditEntry, ...prev].slice(0, 500),
          );
        },
      )
      .subscribe();
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((e) => {
      if (category !== "all" && (e.entity_type || "system") !== category)
        return false;
      if (!q) return true;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-xs text-gray-500">
        <ShieldCheck size={14} /> Immutable audit records · {filtered.length}{" "}
        shown
      </div>
      <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-xs text-gray-500 border-b bg-gray-50 dark:bg-gray-900">
