  ...
  const handlePrintSummary = () => {
    ...
    const esc = (v: unknown) =>
      String(v ?? "")
        ...
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");
    const fuels = fuelTypeApi.activeFuelTypes
      .map(
        (ft: any) =>
          `<tr><td>${esc(ft.localName || ft.canonicalType || "—")}</td><td style="text-align:right">${esc(cur)}${formatNumber(ft.price || 0)}</td><td style="text-align:right">${esc(ft.pumpCount || 0)}</td></tr>`,
      )
      .join("");
    const html = `
    <h1>${esc(companyName)}</h1>
    <h2>Dashboard Summary — ${esc(dateStr)} ${esc(timeStr)}</h2>
    <div style="margin-top:16px">
      <div class="stat"><div class="label">Total Revenue</div><div class="value">${esc(cur)}${formatNumber(totalRevenue || 0)}</div></div>
      <div class="stat"><div class="label">Net Profit</div><div class="value">${esc(cur)}${formatNumber(netProfit || 0)}</div></div>
      <div class="stat"><div class="label">Total Fuel Sold</div><div class="value">${formatNumber(totalFuelSold || 0)} L</div></div>
      <div class="stat"><div class="label">Total Debt</div><div class="value">${esc(cur)}${formatNumber(totalDebt || 0)}</div></div>
    </div>
    ...
    <div class="footer">Generated by FuelPro — ${esc(dateStr)} ${esc(timeStr)}</div>`;

    // Print through the shared pipeline so the browser, Android and desktop
    // shells all use the same path. A popup + setTimeout() call lost the
    // transient user activation on mobile and the dialog silently never
    // opened.
    printHtml(html, {
      title: `${companyName} — Dashboard Summary`,
      paper: "a4",
    }).catch((error) =>
      toastError(
        error instanceof Error ? error.message : "Printing the summary failed",
      ),
    );
  };
