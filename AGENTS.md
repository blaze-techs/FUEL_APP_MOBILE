...
**Gotchas:** (1) Vite folds GameCatalogService into the MAIN index chunk (main.tsx imports it for prefetch) — verify game markers in index-*.js, not the VideoGames lazy chunk. (2) minifier strips strings — check `arcade/data/games` marker. (3) The allorigins.win fuel-price fallback CORS errors on the dashboard are PRE-EXISTING, unrelated. (4) quenq catalog is ~950KB HTML — cache aggressively; never refetch per mount. (5) Don't register beforeunload (bfcache) — the ad-blocker lesson from 62100ab still applies; the game player uses a plain fixed overlay, no unload listeners.### 2026-09-14 follow-up — Video Games perf: PAGE the grid (commit bad3e66, LIVE both hosts)

Manual test on the LIVE hosts (Playwright against production, founder QA user) PROVED a lag bug: rendering all 1,316 game cards at once took **~84s** from tab click to first card (each card = button+img+div subtree; the catalog itself only costs 0.4s to fetch + parse). Fixed by paginating the grid: **PAGE_SIZE=48 cards initially, "Load more" button (+48/click)**, search+genre still scan the full in-memory catalog and reset the window (`useEffect` on `[search, genre]` → `setCount(PAGE_SIZE)`).

Post-fix measurements vs production (both hosts, Playwright):
- tab→first card: **pages.dev 1076ms / vercel 1082ms** (was ~84s → **~78x faster**)
- count label still "1,316 games"; Load more: 48→96 cards; search "soccer" → 1 result in 1.5s
- ZERO game-related console errors (the only console errors are the PRE-EXISTING allorigins.win fuel-price fallback)
...
gameflare: SOLVED in commit 8ee6aea — new `/api/game-embed/gd/` mirror (api/_lib/gamedistribution-embed.ts + CF copy) proxies GameDistribution raw builds (html5.gamedistribution.com/<id>/ , no XFO, no Cloudflare, ACAO *), strips ima3.js/main.min.js (GD ad SDK), injects gdsdk stub firing SDK_READY/SDK_GAME_START → raw ad-free game, ZERO ad requests. juegos.com/poki: still gated (poki-sdk injects runtime ads; juegos 403 ELB) — omitted per NO-ADS.
...
## Session 2026-09-15 (cont.) — GameDistribution/gameflare mirror + CI typecheck repair

- **GameDistribution/gameflare mirror shipped (commit `8ee6aea`)** — new `gd` route in `/api/game-embed/` (Vercel `api/_lib/gamedistribution-embed.ts` + Cloudflare `functions/api/game-embed/[[path]].ts` self-contained copy). Proxies GameDistribution raw builds (`html5.gamedistribution.com/<id>/` — no XFO, no Cloudflare, ACAO `*`), extracts the inner-build prefix from the outer loader's `gameSrc`, 307s to `gd/<prefix>/<id>/index.html`, then strips `ima3.js` + `main.min.js` (GD ad SDK), injects a `gdsdk` stub firing `SDK_READY`/`SDK_GAME_START`, and rewrites absolute `//html5.gamedistribution.com` refs back to the mirror → **raw ad-free game with ZERO ad requests** (canvas verified). Catalog got `GameSource 'gameflare'` + `SOURCE_LABEL/TINT/FILTERS` + unified entries in `GameCatalogService.ts`. 10 unit tests (`src/test/gamedistribution-embed.test.ts`) + `e2e/gamedistribution.spec.ts`. Deployed to CF (`b2330e80`) + Vercel (prod, aliased) — both live serve `/api/game-embed/gd/<id>` → 307 → same-origin inner `index.html` 200 with GET `x-frame-options: SAMEORIGIN`.
- **GitHub "Continuous Integration" workflow was red** (pre-existing, also on `42c4bdd`): `npm run check` = `tsc -b` surfaced two type errors. Fixed in commit `f73ec5c`: removed non-typed legacy JSX attrs `webkitallowfullscreen`/`mozallowfullscreen` from the embed iframe (standard `allowFullScreen` kept → fullscreen still works) + `resolveSwf()` return type `Promise<string | null>` → `Promise<string>` in `api/_lib/quenq-embed.ts` (never returns null; matches CF copy). Also `git rm --cached` generated `coverage/` reports + added `coverage/` to `.gitignore`. Gates now: `tsc -b` exit 0, vitest **447 passed / 6 skipped**, eslint 0 errors, prettier clean on touched files, `build:static` exit 0 (3723 modules).
- **Git push credential note** — the `ghp_` PATs in `/workspace/API KEYS.txt` validate on the REST API (200) but **do not have git-push write scope**; the embedded remote token `ghp_eeKQil...` had **expired (401)** and git prompted for a password. Working push credential: the env `GITHUB_TOKEN` (`ghu_` OAuth token, 40 chars) — `git remote set-url origin https://${GITHUB_TOKEN}@github.com/blaze-techs/FUEL_APP_MOBILE.git` then push succeeds. Keep this in mind for future pushes.
- **Live probe caution** — `curl -sI` (HEAD) on embed routes hits the platform header rule (`X-Frame-Options: DENY` from `vercel.json` catch-all / CF `public/_headers`), but **GET** returns the function's own `X-Frame-Options: SAMEORIGIN` (embedding-safe). Always probe embed routes with GET to assess iframe compatibility. No regression — embedding works on both hosts.
- Remaining (per NO-ADS policy): **juegos.com** (403 AWS ELB) and **poki.com** (CSP `frame-ancestors: https://*.poki.io` + localhosts only) — not yet embeddable ad-free; documented, omitted from catalog.

## Session 2026-09-15 (cont.) — CI fully green + deploy + live E2E verification

- **GitHub "Continuous Integration" is now fully GREEN** (commits `3b0c537` + `a54e9fe`, CI run for `a54e9fe` = success). The last red job was **Lint's prettier stage** (`prettier --check "src/**/*.{ts,tsx}" "*.{json,md}"`) failing on **9 pre-existing files** (8 `src/**` + `TASKS.md`) — fixed with mechanical `prettier --write` only (no logic). tsc 0, vitest 447/6, eslint 0 errors, build exit 0.
- **Deployed GD/gameflare mirror + unified collection to both hosts:**
  - Vercel: `vercel build --prod` exit 0 → `vercel deploy --prebuilt --prod` → aliased `fuel-app-mobile.vercel.app` (chunk `index-DwXcuqx9.js`).
  - Cloudflare: CF Pages auto-deploy of `a54e9fe` (same chunk `index-DwXcuqx9.js`).
  - Both live: `/api/game-embed/gd/5b0abd…` → **307** → `…/gd/rvvASMiM/5b0abd…/index.html` **200**, inner page `main.min.js:0 imasdk:0 gdsdk:1 html5.api:0` (ad-free shim), GET `x-frame-options: SAMEORIGIN`, `access-control-allow-origin: *`, and the built chunk carries the `gameflare` catalog marker (8×) on both hosts.
- **Playwright E2E against live hosts, chromium project, `CI=true` (skips local webServer):**
  - `e2e/gamedistribution.spec.ts` — **2/2 passed** (Vercel + CF): `iframe=true adRequests=0`. Fixed two harness bugs: (1) `page.request.get` **follows redirects by default** so the entry was 200 not 307 → add `{ maxRedirects: 0 }`; (2) `page.goto(data:text/html,…)` **returns null** for data: URLs → assert the DOM (iframe present) instead of a truthy response.
  - `e2e/fullscreen-embed.spec.ts` — **2/2 passed**: `cspMinecraft=true minecraftFrameLoaded=true adRequests=0`.
  - `e2e/quenq-embed.spec.ts` — **2/2 passed**: `canvas=1 ruffle=true adRequests=0 httpFailures=0`.
  - `e2e/crazygames.spec.ts` — **documented auth gap**: the in-app "Games" tab only renders AFTER sign-in (anonymous visitors see the login screen), and the live-host harness has no authenticated `storageState` fixture → the tab-selector times out. NOT a product regression. Raw mirror routes are curl-verified live (`war-the-knights` + `moto-x3m` → 307 → inner 200, 0 ads on both hosts). A valid test account + storageState would unblock the in-app path.
  - `e2e/gamedistribution.spec.ts` + `e2e/crazygames.spec.ts` committed as `d174dc5`.
- **Playwright browser note** — the config's `chromium` project needs the full bundled Chromium; `npx playwright install chromium` downloads `chromium-headless-shell` + full `chromium-1234` + `ffmpeg-1011` (~115 MB). System `/usr/bin/chromium` is NOT used by `channel: "chromium"` in this PW version.
- Remaining per NO-ADS policy: **juegos.com** (403 AWS ELB) and **poki.com** (CSP `frame-ancestors: https://*.poki.io` + localhosts only) — not yet embeddable ad-free; documented, omitted from catalog. **crazygames in-app E2E** blocked on a storageState fixture.

## Session 2026-09-15 (cont.) — In-game CONTROLS system shipped (TASK-2026-09-15-009)

- **Fixed "controls aren't working for all games".** Root cause: the game iframe was never focused, so keyboard/mouse input went to the parent page until the user clicked inside; there was no control-mode selection, no auto-detection, and no controller support for same-origin mirrors.
- **New `src/react-app/lib/game-controls.ts`** — `ControlMode` (auto/keyboard/mouse/gamepad), `detectControls` (polls `navigator.getGamepads()` + pointer media queries), `isSameOriginFrame` (relative `/api/*` mirror paths = same-origin by definition; absolute URLs compared to live origin, false in Node), `focusGameFrame` (autofocus + refocus on surface click), `forwardKeysToFrame` (capture-phase parent key-forwarder so the FIRST keypress reaches the game), `pressedKeysFromGamepad` / `dispatchKeyIntoFrame` (map A/B/X/Y/d-pad/LB/RB/LT/RT/start + sticks → space/e/Shift/q/w/r/f/Enter/arrows + wasd), `startGamepadBridge` (polling gamepad→keyboard bridge, same-origin mirrors only).
- **`VideoGames.tsx` `UnifiedPlayer`** — auto-focus iframe on open/load (with retry), key-forwarder, Controls button in the header with a 4-mode picker (Auto/Keyboard/Mouse/Controller) + live status (gamepad count, touch vs mouse) + "bridge ON" badge, gamepad bridge effect for Auto/Controller modes on same-origin mirrors, one-time "click to focus" hint (auto-dismisses 4.5s), bottom bar status. Cross-origin embeds (archive.org, classic.minecraft.net) keep native gamepad support via the iframe `allow="…gamepad…"` token.
