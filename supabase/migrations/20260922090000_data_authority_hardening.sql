...
VALUES
 ...
 ('fuel_sale','public.sales_ledger','canonical_station_daily_summary',ARRAY['app_kv:FuelContext.salesHistory','app_kv:pos_transactions','localStorage:fuelpro_pos_transactions'],
  'Forecourt fuel sales are immutable in sales_ledger. SalesTracking/POS histories are projections or legacy drafts.'),
 ('product_sale','public.sales_enhanced','SalesInvoices/fetchSales',ARRAY['app_kv:pos_transactions','localStorage:fuelpro_pos_transactions'],
  'General inventory/product POS sales are authoritative in sales_enhanced; POS cache rows are projections.'),
